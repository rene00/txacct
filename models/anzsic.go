...
func (r *anzsicR) GetOrganisations() OrganisationSlice {
	if r == nil {
		return nil
	}
...
func (o *Anzsic) Organisations(mods ...qm.QueryMod) organisationQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"organisation\".\"anzsic_id\"=?", o.ID),
	)
...
func (anzsicL) LoadOrganisations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAnzsic interface{}, mods queries.Applicator) error {
	var slice []*Anzsic
	var object *Anzsic

	if singular {
		var ok bool
		object, ok = maybeAnzsic.(*Anzsic)
		if !ok {
			object = new(Anzsic)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAnzsic)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAnzsic))
			}
		}
	} else {
		s, ok := maybeAnzsic.(*[]*Anzsic)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAnzsic)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAnzsic))
			}
		}
	}
	...
	query := NewQuery(
		...
		qm.WhereIn(`organisation.anzsic_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.AnzsicID) {
				local.R.Organisations = append(local.R.Organisations, foreign)
				if foreign.R == nil {
					foreign.R = &organisationR{}
				}
				foreign.R.Anzsic = local
				break
			}
		}
	}
...
// of the anzsic, optionally inserting them as new records.
// Appends related to o.R.Organisations.
// Sets related.R.Anzsic appropriately.
func (o *Anzsic) AddOrganisations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Organisation) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.AnzsicID, o.ID)
...
// anzsic replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Anzsic's Organisations accordingly.
// Replaces o.R.Organisations with related.
// Sets related.R.Anzsic's Organisations accordingly.
func (o *Anzsic) SetOrganisations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Organisation) error {
	query := "update \"organisation\" set \"anzsic_id\" = null where \"anzsic_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.Organisations {
			queries.SetScanner(&rel.AnzsicID, nil)
			if rel.R == nil {
				continue
			}
...
// Sets related.R.Anzsic.
func (o *Anzsic) RemoveOrganisations(ctx context.Context, exec boil.ContextExecutor, related ...*Organisation) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("anzsic_id")); err != nil {
			return err
		}
	}
	if o.R == nil {
